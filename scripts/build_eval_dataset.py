...
def build_dataset():
    ...
    print("1. Đang tải model HuggingFaceEmbedding (BGE-M3)...")
    embedding_model = HuggingFaceEmbedding(model_name=settings.embedding.huggingface)
    
    print("2. Đang kết nối Qdrant Vector DB...")
    ...
    if not index:
        print("Lỗi: Không lấy được Index!")
        return
    ...
    print(f"3. Bắt đầu tự động dán nhãn (Auto-labeling) cho {len(samples)} câu hỏi...")
    ...
    for i, item in enumerate(samples):
        ...
        try:
            ...
            if (i + 1) % 10 == 0:
                print(f"   Đã xử lý {i + 1}/100 câu hỏi...")
